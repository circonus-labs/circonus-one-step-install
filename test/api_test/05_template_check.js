...
let url_path = "/template/check/system";
...
test(`API ${url_path} (valid)`, function(t) {
    ...
    http.get(url.format(req_obj), (res) => {
        ...
        res.on("end", () => {
            ...
            for (let i = 0; i < requiredCheckProperties.length; i++) {
                t.ok(parsed_data.check.hasOwnProperty(requiredCheckProperties[i]), `check has ${requiredCheckProperties[i]} property`);
            }
    ...
    t.plan(1 + requiredProperties.length + requiredCheckProperties.length);

});


url_path = "/template/check/statsd";

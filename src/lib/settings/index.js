...
/* eslint max-statements: ['error', 125] */
 ...
 * @arg {String} msg message
 * @returns {Undefined} it doesn't it exits...
 */
...
class Settings {
    ...
    constructor() {
        ...
        this.cache_templates = options.cache_templates ||
            cfg.cache_templates ||
            defaults.cache_templates;
        ...
        this.log_keep_max = parseInt(options.log_keep_max ||
            cfg.log_keep_max ||
            defaults.log_keep_max, BASE_10);
        ...
        for (let i = 0; i < requiredBrokerTypes.length; i++) {
            ...
            if (!{}.hasOwnProperty.call(this.default_broker_list, brokerTypeDefault)) {
                this.default_broker_list[brokerTypeDefault] = DEFAULT_BROKER_LIST[brokerTypeDefault]; // eslint-disable-line max-len
            }
        }

        // verify a minimum of "fallback" broker
        if (!({}.hasOwnProperty.call(this.default_broker_list, 'fallback') &&
            {}.hasOwnProperty.call(this.default_broker_list, 'fallback_default'))) {
    ...
    /**
     * print config
     * @arg {Boolean} use_default print default config
     * @returns {Undefined} nothing
     */
    print(use_default) {
        const show_default = use_default !== 'run' || false;

        console.log(JSON.stringify(show_default ? defaults : this, null, 4)); // eslint-disable-line no-restricted-properties
    }
